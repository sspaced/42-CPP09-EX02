...
class PmergeMe {
  ...
  private:
    ...
    std::vector< unsigned int >
    generateJacobsthalSequence(unsigned int maxIndex) const;
    ...
    std::deque< unsigned int >
    generateJacobsthalSequenceDeque(unsigned int maxIndex) const;
	
    // Jacobsthal sequence utility functions
    unsigned int jacobsthalSequence(unsigned int n) const;
};
